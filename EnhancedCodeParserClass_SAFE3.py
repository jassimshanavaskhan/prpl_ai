...
class EnhancedCodeParser:
    ...
    def _parse_complex_struct(self, struct_content: str) -> Dict:
        ...
        while i < len(lines):
            ...
            # Handle union start
            if line.startswith('union'):
                current_union = {'members': [], 'name': self._extract_union_name(line)}
                nested_level += 1
                buffer = []
                i += 1
                continue
            ...
            if line.startswith('struct') and '{' in line:
                current_struct = {'members': [], 'name': self._extract_struct_name(line)}
                nested_level += 1
                buffer = []
                i += 1
                continue
            ...
            if '}' in line:
                ...
                if current_union is not None:
                    ...
                    member = self._create_union_member(current_union, union_name)
                    result['members'].append(member)
                    current_union = None
                elif current_struct is not None:
                    struct_name = self._extract_member_name(line)
                    member = self._create_nested_struct_member(current_struct, struct_name)
                    result['members'].append(member)
                    current_struct = None
                i += 1
                continue
            ...
            if nested_level == 0:
                ...
                if member:
                    ...
                    if self._is_struct_type(member['type']):
                        result['structs_used'].add(self._extract_struct_type(member['type']))
