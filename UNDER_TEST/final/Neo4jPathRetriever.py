...
class Neo4jPathRetriever:
    ...
    def get_complete_chains(self, node_name: str, max_depth: int, node_type: str = None) -> list:
        ...
        """
        print("+++++++++++++++++++++++++++++++++++++++++++")
        print("Getting all Chains for the Node : ")
        ...
        with self.driver.session() as session:
            ...
            print("Node Property : ",name_property)
            ...
            WHERE 
                ...
                ALL(rel IN r1 WHERE type(rel) IN [
                    ...
                    'HAS_NESTED_OBJECT', 'HAS_ACTION',
                    'HAS_VALIDATOR', 'IMPLEMENTED_BY',
                    'USES_STRUCT', 'COMPOSED_OF', 'USES_IN_FUNCTION'  // Added new relationship types
                ]) AND
    ...
    def print_complete_chains(self, chains: list, node_name: str):
        """
        Print the complete chains in a readable format, with enhanced struct relationship display
        ...
        for i, chain in enumerate(chains, 1):
            ...
            # Add a chain summary for struct relationships
            struct_steps = [step for step in chain["path_sequence"] 
                        if step['relationship'] in ['USES_STRUCT', 'COMPOSED_OF', 'USES_IN_FUNCTION']]
            if struct_steps:
                print("  [Chain contains struct relationships]")
            ...
            for step in chain["path_sequence"]:
                ...
                rel_type = ""
                if relationship in ['USES_STRUCT', 'COMPOSED_OF', 'USES_IN_FUNCTION']:
                    rel_type = " (Struct Relationship)"
                
                # Format line number info
                line_info = f":line {step['line_number']}" if step['line_number'] is not None else ""
                
                # Print the step with enhanced formatting
                print(f"  {step['from']['type']}({from_highlight}{step['from']['name']}{from_highlight}) "
                    f"--[{relationship}{line_info}{rel_type}]--> "
                    f"{step['to']['type']}({to_highlight}{step['to']['name']}{to_highlight})")
                
            # Add summary at the end of each chain if it contains struct relationships
            if struct_steps:
                print("  Struct relationships in this chain:")
                for step in struct_steps:
                    print(f"    • {step['from']['name']} {step['relationship']} {step['to']['name']}")
