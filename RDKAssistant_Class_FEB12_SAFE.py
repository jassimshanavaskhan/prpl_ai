...
class RDKAssistant:
    ...
    def handle_user_interaction(self):
        ...
        while True:
            ...
            print("5. Exit")  # Updated numbering
    ...
    #         # Neo4j Integration: Create Code Entity Nodes
    #         neo4j_processor.create_code_entities(processed_entities)
            
    #         # Neo4j Integration: Create Function Call Relationships
    #         neo4j_processor.create_function_call_relationships(processed_entities)
            
    #         # Optional: Retrieve Function Call Paths (for debugging or analysis)
    #         # paths = neo4j_processor.retrieve_function_call_paths(processed_entities)
    ...
    #         else:
    #             files = []
    #             # Existing code file extensions
    #             code_extensions = ['.c', '.cpp', '.h', '.cc']
    #             # Add ODL file extension
    ...
    #         processed_entities = []
    #         odl_entities = {}  # Dictionary to store ODL parsing results
    ...
    #                 # Store ODL result for vector creation
    #                 odl_entities[str(file_path)] = odl_result
                    
    #                 # Map ODL to Neo4j
    #                 # ================================================================ SAFE
    #                 # odl_neo4j_mapper.map_definition_to_neo4j(odl_result, original_content)
    #                 # ================================================================ SAFE
    #                 # ================================================================ MODIFIED
    ...
    #                 # # Verify callback matches (optional)
    #                 # unmatched_callbacks = odl_neo4j_mapper.verify_callback_matches()
                    
    #                 # # You might want to create a CodeEntity for the ODL file itself
    #                 # odl_entity = CodeEntity(
    #                 #     name=f"ODL_{file_path.stem}",
    #                 #     type="ODL_DEFINITION",
    #                 #     content=original_content,
    #                 #     file_path=str(file_path),
    #                 #     component=component_name
    #                 # )
    #                 # processed_entities.append(odl_entity)
                    
    #             else:
    #                 # Existing code file processing
    #                 entities = self._process_single_file(file_path, component_name)
    ...
    #         # neo4j_processor.create_function_call_relationships(processed_entities)
    #         #======================================== SAFE
    #         # Process all relationships (both function calls and struct usage)
    ...
    #             odl_entities=odl_entities  # Pass ODL entities to vector store
    #         )
    ...
    #         logger.info("Processed Entities Summary:")
    #         # for entity in processed_entities:
    #         #     print("\n--- Entity Details ---")
    #         #     print(f"Name: {entity.name}")
    #         #     print(f"Type: {entity.type}")
    #         #     print(f"File Path: {entity.file_path}")
    #         #     print(f"Component: {entity.component}")
                
    #         #     # Print additional metadata for structs
    #         #     if entity.type == 'struct' and hasattr(entity, 'metadata'):
    #         #         print("\nStruct Metadata:")
    #         #         for key, value in entity.metadata.items():
    #         #             print(f"  {key}: {value}")
                
    #         #     # If you want a more detailed JSON-like representation
    #         #     print("\nFull Entity Representation:")
    #         #     print(json.dumps(entity.__dict__, indent=2))
            
    #         # print(f"\nTotal Processed Entities: {len(processed_entities)}")
    ...
    # def search_relevant_entities(self, query: str) -> List[CodeEntity]:
    #     """
    #     Search for relevant code entities using Neo4j first, 
    #     with Faiss as a fallback mechanism
        
    #     :param query: Search query string
    #     :return: List of relevant code entities
    #     """
    #     try:
    #         # First, attempt to search using Neo4j-based method
    #         neo4j_results = self.neo4j_context_retriever.search_functions(query)
            
    #         # If Neo4j search returns results, convert and return
    #         if neo4j_results:
    #             neo4j_entities = []
    #             for result in neo4j_results:
    #                 # Create a CodeEntity from Neo4j result
    #                 entity = CodeEntity(
    #                     name=result['name'],
    ...
    #             logger.info(f"Found {len(neo4j_entities)} entities via Neo4j search")
    #             return neo4j_entities
            
    #         # Fallback to Faiss if Neo4j search yields no results
    #         logger.info("No results from Neo4j, falling back to Faiss")
    ...
    #         # Combine and deduplicate Faiss results
    #         relevant_entities = set()
    #         for result in relevant_functions + relevant_structs + relevant_apis:
    #             entity_name = result['metadata']['name']
    #             if entity_name in self.entities:
    ...
    #         logger.info(f"Found {len(relevant_entities)} entities via Faiss search")
    #         return list(relevant_entities)
