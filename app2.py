...
class AdaptiveSearchSelector:
    ...
    def select_best_result(self, advanced_search, user_query, initial_strategy=SearchStrategy.VECTOR):
        ...
        for strategy in strategies:
            ...
            logger.info(f"[PRPL_ASSIST_LOG] Tring Search Strategy : {strategy.name}")
            ...
            for idx, result in enumerate(search_results, 1):
                print(f"\nResult {idx}:")
                logger.info(f"[PRPL_ASSIST_LOG] Search Result Name : {result.content}")
...
class QueryClassifier:
    ...
    def classify_query(self, query: str) -> Dict[str, str]:
        ...
        try:
            logger.info(f"[PRPL ASSIST] Created Prompt for Query Classification : ")
            logger.info(f"[PRpL ASSIST] : {prompt}")
...
def create_app(config):
    ...
    def chat():
        ...
        try:
            ...
            logger.info(f"[PRPL_ASSIST_LOG] Query Reached at Server : {query}")
            print("======================= >> Query Reached at Server ")
            print("======================= >> Query Calssifying.... ")
            query_classifier = QueryClassifier(os.environ.get('GEMINI_API_KEY'))
            logger.info(f"[PRPL_ASSIST_LOG] Query Classification Started...")
            query_classification = query_classifier.classify_query(query)
            print("======================= >> Classification Decision : ")
            logger.info(f"[PRPL_ASSIST_LOG] Classified Query : ")
            # logger.info(f"Query Type : ",query_classification['query_type'])
            logger.info(f"[PRPL_ASSIST_LOG] Query Type : {query_classification['query_type']}")
            logger.info(f"[PRPL_ASSIST_LOG] Reasonong : {query_classification['reasoning']}")
            logger.info(f"[PRPL_ASSIST_LOG] Confidence Score : {query_classification['confidence_score']}")
            ...
            if query_classification['query_type'] == 'general':
                ...
                logger.info(f"[PRPL_ASSIST_LOG] Following General Path")
            ...
            logger.info(f"[PRPL_ASSIST_LOG] Following Technical Path")
            ...
            if paths:
                ...
                # When paths are None, use the general response generator
                logger.info(f"[PRPL_ASSIST_LOG] No valid paths found, falling back to General Path")
                response_generator = GeneralResponseGenerator(vector_store)
                response = response_generator.generate_comprehensive_response(query)
...
if __name__ == '__main__':
    ...
    config = {
        ...
        'advanced_search': AdvancedCodeSearch(
            vector_store,
