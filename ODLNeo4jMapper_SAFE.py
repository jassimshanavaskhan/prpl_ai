...
class ODLNeo4jMapper:
    ...
    def create_odl_node(self, filepath: str, content: str) -> str:
        """Create an ODL node with content and return its identifier"""
        with self.driver.session() as session:
            result = session.run("""
                CREATE (odl:ODL {
                    filepath: $filepath,
    ...
    def map_definition_to_neo4j(self, definition_info: Dict[str, Any], odl_content: str):
        """Map the entire ODL definition to Neo4j including the ODL content"""
        # Create the main ODL node with content
        odl_id = self.create_odl_node(definition_info["filepath"], odl_content)
        
        # Create Object nodes and their parameters
        for object_name, object_info in definition_info["objects"].items():
            ...
            # Create Parameter nodes for the object
