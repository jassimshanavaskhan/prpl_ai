...
class VectorStoreManager:
    ...
    def __init__(self, embedding_model: GooglePalmEmbeddings):
        ...
        self.vector_stores = {
            ...
            'odl_object': None,
            'odl_file': None,
            'documentation': None,  # New store for documentation
            'web_content': None     # New store for web content
        }
        self.text_splitter = RecursiveCharacterTextSplitter(
            chunk_size=1000,
            chunk_overlap=200
    ...
    ############################################################################### SAFE
    def save_indices(self, base_path: str = "vector_stores"):
        """Save all vector store indices with metadata"""
        os.makedirs(base_path, exist_ok=True)
        ...
        for store_type, store in self.vector_stores.items():
            if store is not None:
                ...
                logger.info(f"Saved {store_type} index with {metadata['total_vectors']} vectors")
    ...
    def add_documents(self, documents: List[Dict]):
        ...
        if self.vector_stores['documentation'] is None:
            self.vector_stores['documentation'] = FAISS.from_texts(
                texts=texts,
                embedding=self.embedding_model,
                metadatas=metadatas
            )
        else:
            self.vector_stores['documentation'].add_texts(
                texts=texts,
                metadatas=metadatas
            )
    ...
    def add_web_content(self, web_content: List[Dict]):
        """Add web content to vector store"""
        texts = [content['content'] for content in web_content]
        metadatas = [content['metadata'] for content in web_content]
        
        if self.vector_stores['web_content'] is None:
            self.vector_stores['web_content'] = FAISS.from_texts(
                texts=texts,
                embedding=self.embedding_model,
                metadatas=metadatas
            )
        else:
            self.vector_stores['web_content'].add_texts(
                texts=texts,
                metadatas=metadatas
            )
