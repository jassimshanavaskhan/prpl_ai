...
class EnhancedCodeParser:
    ...
    def _create_function_entity_from_dict(
        ...
        try:
            ...
            # 1. Check return type for struct
            if return_type.startswith('struct '):
                structs_used.add(return_type.split()[1])
            
            # 2. Check parameters for struct types
            for param in parameters:
                # Parameters should now be properly parsed dictionaries
                param_type = param['type'] if isinstance(param, dict) else param
                if isinstance(param_type, str):
                    if param_type.startswith('struct '):
                        structs_used.add(param_type.split()[1])
                    elif self._is_struct_type(param_type):
                        structs_used.add(self._extract_struct_type(param_type))
            
            # 3. Analyze function body for struct usage
            body_structs = self._analyze_function_body_for_structs(function_content)
            structs_used.update(body_structs)
            ...
            return CodeEntity(
                ...
                metadata={
                    ...
                    'context_before': context_before,
                    'context_after': context_after,
                    'structs_referenced': list(structs_used)  # Also add to metadata for consistency
                }
            )
    ...
    def _analyze_function_body_for_structs(self, function_content: str) -> Set[str]:
        """Analyze function body to find struct usage."""
        structs_used = set()
        try:
            # Remove string literals to avoid false positives
            content = re.sub(r'".*?"', '""', function_content)
            content = re.sub(r"'.*?'", "''", content)
            
            # Find struct declarations and usage
            patterns = [
                # Struct variable declarations
                r'struct\s+(\w+)\s+\w+\s*[;=]',
                # Struct pointer declarations
                r'struct\s+(\w+)\s*\*\s*\w+\s*[;=]',
                # Typedef'd struct usage (common patterns)
                r'\b(\w+_t)\s+\w+\s*[;=]',
                r'\b(\w+_st)\s+\w+\s*[;=]',
                # Struct member access
                r'(?:->|\.)\s*(\w+)',
                # Casting to struct pointer
                r'\(\s*struct\s+(\w+)\s*\*?\s*\)',
                # sizeof struct
                r'sizeof\s*\(\s*struct\s+(\w+)\s*\)',
            ]
            ...
            for pattern in patterns:
                ...
                for match in matches:
                    struct_name = match.group(1)
                    # Verify if it's actually a struct type
                    if (struct_name.endswith('_t') or 
                        struct_name.endswith('_st') or 
                        struct_name in self.known_struct_types):
                        structs_used.add(struct_name)
            
            # Check for typedef'd struct names in declarations
            for struct_type in self.known_struct_types:
                pattern = fr'\b{struct_type}\b\s+\w+\s*[;=]'
                if re.search(pattern, content):
                    structs_used.add(struct_type)
            
            # Find structs in array declarations
            array_pattern = r'\b(struct\s+\w+|\w+_t|\w+_st)\s+\w+\s*\['
            for match in re.finditer(array_pattern, content):
                type_name = match.group(1)
                if type_name.startswith('struct '):
                    structs_used.add(type_name.split()[1])
                elif self._is_struct_type(type_name):
                    structs_used.add(type_name)
            
            return structs_used
            
        except Exception as e:
            logger.error(f"Error analyzing function body for structs: {str(e)}")
            return set()
